class MarkdownEngine {
    #engine
    constructor () {
        this.#engine = require("markdown-it")
    }
    use (component) {
        component.run(this.#engine)
    }
}

// 封装 CodeMirror 及其相关逻辑
class Editor {}

// 封装界面预置的组件，在页面加载完成后，初始化 Markdown 编辑器
// 与编辑快捷键和上下文菜单对接
class UserInterface {
    #editor
    #markdownView
    ...
    async init () {
        ...
        this.#markdownView = document.querySelector("#markdown")
        this.#htmlView = document.querySelector("#html")
    }
    constructor () {
        this.init()
    }
...
// 应用程序本身
// 初始化用户界面，进入可编辑状态后，以异步方式引入各类次生组件
class Application {
    #engine
    #userInterface
    constructor () {
...
// Markdown 组件的抽象类，其派生类描述某一类用途的组件，实例为具体的单个组件
class MarkdownComponent {
     ...
     * 在需要使用时才注册
     */
    register (packageWrapper, configuration ) {
        if (!configuration && typeof packageWrapper === "object") {
            this.#configuration = packageWrapper
            this.#package = require(this.#name)
        } else {
            this.#package = packageWrapper(this.#name)
            this.#configuration = configuration || {}
        }
    }
    run (engine) {
        engine.use(this.#package, this.#configuration)
    }

}

class MarkdownFormula extends MarkdownComponent {
    engineTypes = ["KaTeX", "MathJax"]
}
class MarkdownCodeblock extends MarkdownComponent {}
class MarkdownEmoji extends MarkdownComponent {}
class MarkdownMedia extends MarkdownComponent {}
class MarkdownTable extends MarkdownComponent {}
class MarkdownText extends MarkdownComponent {}

const markdownFormula = new MarkdownFormula("markdown-it-texmath")
markdownFormula.register({
